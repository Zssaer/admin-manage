      ...
      <ul class="layout-navbars-tagsview-ul" :class="setTagsStyle">
        <li v-for="(v, k) in tagsViewList" :key="k" class="layout-navbars-tagsview-ul-li" :data-name="v.name" :class="{ 'is-active': isActive(v.path) }" @contextmenu.prevent="onContextmenu(v, $event)" @click="onTagsClick(v, k)" :ref="(el) => { if (el) tagsRefs[k] = el; }">
          ...
          <template v-if="isActive(v.path)">
            ...
            <i class="el-icon-close layout-navbars-tagsview-ul-li-icon layout-icon-active" v-if="!v.meta.isAffix" @click="closeCurrentTagsView(v.path)"></i>
          </template>
          <i class="el-icon-close layout-navbars-tagsview-ul-li-icon layout-icon-three" v-if="!v.meta.isAffix" @click="closeCurrentTagsView(v.path)"></i>
        </li>
      </ul>
    </Scroll>
  </div>
...
export default {
  ...
  setup() {
    ...
    const setTagsStyle = computed(() => {
      let { tagsStyle } = store.state.themeConfig.themeConfig;
